...
def collate_fn_wav(batch):
    ...
    ## compute mask
    return total_wav, attention_mask

def collate_fn_wav_env(batch):
    total_wav = []; total_dur = []; total_env = []
    ...
    total_env = torch.Tensor(np.array(total_env))
    ## compute mask
    return total_wav, total_env, attention_mask

def collate_fn_wav_env_type(batch):
    total_wav = []; total_dur = []; total_env = []
    for wav_data in batch:
        wav, dur = wav_data[0]
        env = wav_data[1]
...
def collate_fn_wav_lab_mask_env(batch):
    total_wav = []
    total_lab = []
    total_dur = []
    total_env = []
    ...
    total_env = torch.Tensor(np.array(total_env))
    ## compute mask
    return total_wav, total_lab, attention_mask, total_env

def collate_fn_wav_lab_mask_env_type(batch):
    total_wav = []
    total_lab = []
    total_dur = []
    total_env = []
