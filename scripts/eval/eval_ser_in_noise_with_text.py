...
text_model = AutoModel.from_pretrained("roberta-large")
...
parser.add_argument("--txt_type", type=str, choices=["bert", "clap", "glove"], default="bert")
...
audio_path = os.path.join(corpus_path, str(args.seed))
...
for dtype in ["test"]:
    ...
    noisy_wavs = utils.load_audio(audio_path, cur_utts)
...
default_template = "This voice is recorded with a sound of "
template_list = [
    "",
...
template = template_list[int(args.template_idx)] if args.template_idx is not None else default_template


import json
env_desc_dict = json.load(open(os.path.join(corpus_path, "env_desc", f"{args.seed}.json")))
...
for xy_pair in tqdm(total_dataloader["test"]):
    ...
    with torch.no_grad():
        env_desc = template + ", ".join(env_desc_dict[cur_utts[idx]].split("_"))
        inputs = tokenizer(text=env_desc, return_tensors="pt").to(0)
        env_x =  text_model(**inputs).pooler_output
