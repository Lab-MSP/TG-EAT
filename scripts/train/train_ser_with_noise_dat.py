...
for dtype in ["train", "dev"]:
    ...
    domain_dict = noise_idx_set.noise_idx_dict if dtype != "train" else None
    
    noise_idx_set = utils.NoiseIdxSet(noise_type_list, noise_idx_dict=domain_dict)
    ...
    noise_dataset = utils.CombinedSet([noise_wav_set, noise_idx_set])
    noise_loader[dtype] = DataLoader(
        noise_dataset, batch_size=cur_bs, shuffle=is_shuffle, 
        pin_memory=True, num_workers=4,
        collate_fn=utils.collate_fn_wav_env)
...
domain_model = net.EmotionRegression(feat_dim, 512, 1, DOMAIN_NUM, p=0.5)
domain_model.eval(); domain_model.cuda()
...
domain_opt = torch.optim.Adam(domain_model.parameters(), LR)
...
domain_sch = optim.lr_scheduler.LambdaLR(domain_opt, lr_lambda=lambda step: warmup_lambda(step, warmup_steps))

lm = utils.LogManager()
lm.alloc_stat_type_list(["train_aro", "train_dom", "train_val"])
lm.alloc_stat_type_list(["dev_aro", "dev_dom", "dev_val"])
lm.alloc_stat_type_list(["train_domain", "dev_domain"])
...
for epoch in range(EPOCHS):
    ...
    for xy_pair in tqdm(total_dataloader["train"]):
        ...
        env_y=env_x.cuda(non_blocking=True).long()
        ...
        with autocast(enabled=True):
            ...
            ssl = torch.mean(noisy_ssl, dim=1)
            
            domain_pred = domain_model(ssl)
            emo_pred = ser_model(ssl)
            ccc = utils.CCC_loss(emo_pred, y)
            loss = 1.0 - ccc
            
            domain_loss = F.nll_loss(domain_pred, env_y)
        ...
        if (batch_cnt+1) % ACCUMULATION_STEP == 0 or (batch_cnt+1) == len(total_dataloader["train"]):
            ...
            domain_sch.step()  # Update learning rate
        ...
        lm.add_torch_stat("train_domain", domain_loss)   
    ...
    lm.add_torch_stat("dev_domain", domain_loss)
    ...
    torch.save(domain_model.state_dict(), \
        os.path.join(MODEL_PATH, "param", str(epoch)+"_domain.pt"))
    ...
    tb_writer.add_scalar('Train/Domain', lm.get_stat("train_domain"), epoch)
    tb_writer.add_scalar('Dev/Domain', lm.get_stat("dev_domain"), epoch)
