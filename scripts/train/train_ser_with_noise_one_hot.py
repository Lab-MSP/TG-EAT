...
parser.add_argument("--fuse_type", type=str, choices=["RT", "RH", "SCA", "RH-avg"], default="RT")
...
env_opt = torch.optim.Adam(env_model.parameters(), LR)
...
env_sch = optim.lr_scheduler.LambdaLR(env_opt, lr_lambda=lambda step: warmup_lambda(step, warmup_steps))
...
for epoch in range(EPOCHS):
    ...
    for xy_pair in tqdm(total_dataloader["train"]):
        ...
        env_x=env_x.cuda(non_blocking=True).long()
        ...
        if (batch_cnt+1) % ACCUMULATION_STEP == 0 or (batch_cnt+1) == len(total_dataloader["train"]):
            ...
            env_sch.step()  # Update learning rate
    ...
    torch.save(env_model.state_dict(), \
        os.path.join(MODEL_PATH, "param", str(epoch)+"_env.pt"))
