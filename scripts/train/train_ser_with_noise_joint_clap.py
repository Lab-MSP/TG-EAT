...
lm = utils.LogManager()
lm.alloc_stat_type_list(["train_aro", "train_dom", "train_val", "train_cos"])
lm.alloc_stat_type_list(["dev_aro", "dev_dom", "dev_val", "dev_cos"])
...
for epoch in range(EPOCHS):
    ...
    for xy_pair in tqdm(total_dataloader["train"]):
        ...
        with autocast(enabled=True):
            # Resample to 48kHz
            clap_aud_embs = []
            for cur_x in noisy_x:
                clap_noisy_x = librosa.resample(cur_x.numpy(), orig_sr=16000, target_sr=48000)
                clap_noisy_x = torch.Tensor(clap_noisy_x)
                aud_x = processor(audios=clap_noisy_x, sampling_rate=48000, return_tensors="pt").to(0)
                aud_env_x = clap_txt_model.get_audio_features(**aud_x)
                clap_aud_embs.append(aud_env_x)
            aud_env_x = torch.cat(clap_aud_embs, dim=0)
            
            clap_txt_embs = []
            for cur_x in env_x:
                clap_inputs = processor(text=cur_x, return_tensors="pt").to(0)
                clap_emb = clap_txt_model.get_text_features(**clap_inputs)
                clap_txt_embs.append(clap_emb)
            env_x = torch.cat(clap_txt_embs, dim=0)            
            
            # Cosine similarity
            noisy_x=noisy_x.cuda(non_blocking=True).float()
            
            audio_features = aud_env_x
            text_features = env_x
            ...
            labels = torch.arange(num_logits, device="cuda:0", dtype=torch.long)
            cosine_loss = (F.cross_entropy(logits_per_audio, labels)+F.cross_entropy(logits_per_text, labels))/2
        ...
        lm.add_torch_stat("train_cos", cosine_loss)  
    ...
    for xy_pair in tqdm(total_dataloader["dev"]):
        ...
        with torch.no_grad():
            ...
            lm.add_torch_stat("dev_cos", cosine_loss)  
